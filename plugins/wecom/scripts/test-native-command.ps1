...
$nativeArguments = if (-not [string]::IsNullOrWhiteSpace($env:ComSpec)) {
    @('/d', '/s', '/c', 'echo normal-output & echo normal-qr-progress 1>&2 & exit /b 3')
} else {
    @('-c', 'echo normal-output; echo normal-qr-progress >&2; exit 3')
}
...
Write-Host 'WeCom native-command boundary test passed.'
