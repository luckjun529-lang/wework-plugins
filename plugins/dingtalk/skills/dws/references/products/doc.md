...
dws doc create --name "项目周报" --content-file <temp-dir>/x.md \
...
dws doc read --node <DOC_ID> --content-format jsonml --output <temp-dir>/doc.json   # 1. 读出当前 JSONML
# 修改 <temp-dir>/doc.json 中的 jsonml 数组
dws doc update --node <DOC_ID> --content-file <temp-dir>/doc.json \
...
dws doc export --node <NODE_ID> --output <output-dir>          # contentType=ALIDOC 走 export（内容层）
dws drive download --node <NODE_ID> --output <output-dir>      # contentType≠ALIDOC 走 drive download（存储层）
