...
- **解析输出时只读 stdout**：CLI 把数据 JSON 写到 stdout、把诊断与警告写到 stderr。解析 JSON 时**不要合并这两条流**，否则警告行混进 JSON 会让解析失败。通过包装器读取 stdout 并在内存中解析；需要诊断信息时单独读取 stderr，不依赖 shell 管道或重定向。
